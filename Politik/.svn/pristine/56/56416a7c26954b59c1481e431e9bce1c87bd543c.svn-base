...
import com.saganet.politik.database.encuestas.hermanos.EntrevistadorEO;
import com.saganet.politik.dominios.NivelesDO;

@Component("HermanosUsuarioC")
public class RegistroUsuarioC {
	...
	public String validarDatos(RequestContext context, EntrevistadorEO visitador) {
		...
		filas = sqlSession.selectOne("encuestas.hermanos.registroUsuarios.validarCodigo", mapa);
		if (filas == 0) {
	...
	public UsuarioEO registrar(EntrevistadorEO visitador) {
		HashMap<String, Object>  valorRegistro;
		UsuarioEO usuario;
		...
		valorRegistro =  sqlSession.selectOne("encuestas.hermanos.registroUsuarios.datosCodigo", visitador.getClave());
		logger.debug("despues Valor registro : {}", valorRegistro);
			visitador.setPrograma(valorRegistro.get("programa").toString());
		////informacion del usuario
		
		rol = new RolEO();
		rol.setRol("ROLE_ENTREVISTA_HERMANOS");
		listado.add(rol);
		...
		listado.add(rol);
		// crear usuario
		usuario.setNick("eh_" + sqlSession.selectOne("encuestas.hermanos.registroUsuarios.conteoUsuario"));
		...
		sqlSession.insert("encuestas.hermanos.registroUsuarios.insertarEncuestadores", visitador);
