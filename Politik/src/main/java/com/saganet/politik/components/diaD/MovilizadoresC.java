...
public class MovilizadoresC {
	...
	public void listadoMovilizados(MovilizadorEO movilizador) {
		logger.debug("Función listado Movilizados: {}", movilizador);
		promovidos.setMovilizador(movilizador);
		logger.debug("Movilizador componentee: {}", movilizador);
	}
	public void listadoMovilizadosPorFolio(MovilizadorEO movilizador) {
		List<MovilizadoEO> listado=sqlSession.selectList("diaD.movilizado.listadoPorMovilizadorOrdenFolio", movilizador.getId());
		movilizador.setListadoMovilizado(listado);
		promovidos.setMovilizador(movilizador);
	}
	
	public void listadoMovilizadosPorNombre(MovilizadorEO movilizador) {
		List<MovilizadoEO> listado=sqlSession.selectList("diaD.movilizado.listadoPorMovilizadorOrdeNombre", movilizador.getId());
		movilizador.setListadoMovilizado(listado);
		promovidos.setMovilizador(movilizador);
	}
	
	public void listadoMovilizadosFoliosPorFolio(MovilizadorEO movilizador) {
		List<MovilizadoEO> listado=sqlSession.selectList("diaD.movilizado.listadoPorMovilizadorOrdenFolio", movilizador.getId());
		movilizador.setListadoMovilizado(listado);
		promovidos.setMovilizador(movilizador);
	}
	
	public void listadoMovilizadosFoliosPorNombre(MovilizadorEO movilizador) {
		List<MovilizadoEO> listado=sqlSession.selectList("diaD.movilizado.listadoPorMovilizadorOrdeNombre", movilizador.getId());
		movilizador.setListadoMovilizado(listado);
		promovidos.setMovilizador(movilizador);
	}
